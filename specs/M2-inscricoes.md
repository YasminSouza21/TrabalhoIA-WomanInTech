...
## 1. Objetivo
Permite que participantes se inscrevam em atividades da Semana Acadêmica 2026, entrem na lista de espera quando lotadas, sejam convocados automaticamente quando uma vaga abrir e confirmem a convocação dentro de um prazo. Participantes e organização consultam inscrições; apenas participantes mutam as próprias.
...
- Interface mínima Flutter é só registrada, sem implementação nesta rodada. Origem: P-34.
...
## 5. Regras
- R01. Toda rota do M2 exige `X-Usuario`; sem cabeçalho ou id inexistente → `401 USUARIO_DESCONHECIDO` primeiro. Origem: F-06.
- R02. Rotas de participante (inscrever, cancelar, confirmar) recusam organização → `403 SOMENTE_PARTICIPANTE`. Origem: F-08.
- R03. Atividade ou inscrição inexistente → `404 NAO_ENCONTRADO` antes das regras do recurso. Origem: F-09.
...
- R05. Ordem das verificações: identificação (401) → perfil (403) → existência (404) → corpo (422) → regras do recurso. Origem: F-07.
...
- R31. Confirmar convocação, em ordem: `SEM_CONVOCACAO` → `CONVOCACAO_EXPIRADA` → `CONFLITO_DE_HORARIO` → `LIMITE_DE_MINICURSOS`; convocada vencida pelo relógio retorna `CONVOCACAO_EXPIRADA` mesmo depois de leitura já ter observado o vencimento. Origem: P-20.
- R32. Confirmar inscrição sem convocação ativa → `422 SEM_CONVOCACAO`. Origem: contrato-api.md.
...
- R34. Cancelar inscrição de atividade já iniciada → `422 ATIVIDADE_JA_INICIADA`. Origem: F-13.
...
18. (R31, R32) Confirmar sem convocação → `SEM_CONVOCACAO`; convocada vencida → `CONVOCACAO_EXPIRADA`.
